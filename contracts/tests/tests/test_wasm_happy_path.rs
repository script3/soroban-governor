...
use tests::governor::{create_soroban_admin_governor_wasm, create_soroban_governor_wasm};
use tests::votes::AdminVotesClient;
use tests::{
    env::EnvTestUtils,
...
#[test]
fn test_wasm_happy_path_admin_token() {
    let e = Env::default();
    e.set_default_info();
    e.mock_all_auths();
    e.budget().reset_unlimited();
    ...
    let settings = default_governor_settings(&e);
    let (token_address, token_client) = create_stellar_token(&e, &bombadil);
    let (governor_address, votes_address) =
        create_soroban_admin_governor_wasm(&e, &bombadil, &settings);
    let votes_client = AdminVotesClient::new(&e, &votes_address);
    let governor_client = GovernorContractClient::new(&e, &governor_address);
    let (subcall_address, _) =
        create_mock_subcall_contract_wasm(&e, &token_address, &governor_address);
    ...
    // pass some time - samwise delegates to frodo then admin clawbacks tokens from samwise. Merry mints more votes.
    e.jump(settings.vote_delay - ONE_HOUR);
    ...
    frodo_votes -= 1000 * 10i128.pow(7);
    votes_client.clawback(&samwise, &(1000 * 10i128.pow(7)));
    total_votes -= 1000 * 10i128.pow(7);
    let mut merry_votes = 0;
    ...
    // merry tries to delegate votes to pippin after the delay and mint more votes
    // the admin then clawbacks from merry
    // @dev: don't update vote trackers with changes after the vote period starts
    votes_client.delegate(&merry, &pippin);
    let late_mint_votes = 500 * 10i128.pow(7);
    let late_clawback_votes = 1000 * 10i128.pow(7);
    votes_client.mint(&merry, &late_mint_votes);
    votes_client.clawback(&merry, &late_clawback_votes);
    assert_eq!(votes_client.get_votes(&merry), 0);
    assert_eq!(
        votes_client.get_votes(&pippin),
        pippin_votes + merry_votes + late_mint_votes - late_clawback_votes
    );
    assert_eq!(
        votes_client.total_supply(),
        total_votes + late_mint_votes - late_clawback_votes
    );
