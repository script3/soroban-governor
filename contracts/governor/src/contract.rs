...
mod tests {
    ...
    fn test_propose() {
        ...
        let calldata = CallData{ contract_address: Address::generate(&e), function: String::from_str(&e, "test"), args: (1, 2, 3).into_val(&e) };
        let sub_calldata = &vec![&e,SubCallData{ contract_address: Address::generate(&e), function: String::from_str(&e, "test"), args: (1, 2, 3).into_val(&e), sub_auth: vec![&e] }];
        ...
        e.as_contract(&govenor_address, || {
            ...
            let status = storage::get_proposal_status(&e, 0).unwrap();
            assert_eq!(proposal.calldata.function, calldata.function);
            assert_eq!(proposal.calldata.contract_address, calldata.contract_address);
            assert_eq!(proposal.calldata.args, calldata.args);
            assert_eq!(proposal.sub_calldata.get(0).unwrap().contract_address, sub_calldata.get(0).unwrap().contract_address);
            assert_eq!(proposal.sub_calldata.get(0).unwrap().function, sub_calldata.get(0).unwrap().function);
            assert_eq!(proposal.sub_calldata.get(0).unwrap().args, sub_calldata.get(0).unwrap().args);
            assert_eq!(proposal.sub_calldata.get(0).unwrap().sub_auth.len(), sub_calldata.get(0).unwrap().sub_auth.len());
            assert_eq!(proposal.proposer, creater);
            assert_eq!(proposal.title, title);
            assert_eq!(proposal.description, description);
            assert_eq!(proposal.vote_start, settings.vote_delay);
            assert_eq!(proposal.vote_end, settings.vote_delay + settings.vote_period);
